...
<head>
    ...
    <script>
        ...
        window.onload = function() {
            var membership = "${sessionScope.membership}"; // 현재 회원 등급 가져오기
            if (membership === "VIP") {
            alert("⚠ 이미 VIP 등급입니다!"); // 팝업(alert) 표시
            window.location.href = "index"; // 홈 화면으로 이동
          }
    };
    </script>
</head>
<body>
    <div class="container">
        <h2>VIP 멤버십 결제</h2>
        <form id="paymentForm" action="vipPaymentProc" method="post">
            <label>카드번호:</label>
